...
function startWebpackEntcore(isLocal) {
    var streams = [];
    apps.forEach(a => {
        var str = gulp.src('./' + a + '.src/main/resources/public')
...
gulp.task('copy-local-libs', ['drop-old-files'], () => {
    ...
    apps.forEach(a => {
        ...
        streams.push(html, ts, module, toolkitModule);
    });
    return merge(streams);
});
...
gulp.task('update-libs', ['bower'], function(){
    var streams = [];
    apps.forEach(a => {
        var html = gulp.src('./' + a + '/bower_components/entcore/src/template/**/*.html')
            .pipe(gulp.dest('./' + a + '/src/main/resources/public/template/entcore'));

        var ts = gulp.src('./' + a + '/bower_components/entcore/src/ts/**/*.ts')
            .pipe(gulp.dest('./' + a + '/src/main/resources/public/ts/entcore'));

        var module = gulp.src('./' + a + '/bower_components/entcore/src/ts/**/*.ts')
            .pipe(gulp.dest('./' + a + '/node_modules/entcore'));

        var sassJs = gulp.src('./' + a + '/bower_components/sass.js/dist/**/*.js')
            .pipe(gulp.dest('./' + a + '/src/main/resources/public/dist/sass-js'));
            
        streams.push(html, ts, sassJs);
    });
    return merge(streams);
});
